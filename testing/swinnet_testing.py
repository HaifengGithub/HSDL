...
def testing(testtxtdir, modeldir, savedir):
    ...
    np.save(os.path.join(savedir,'swinnet_results.npy'),
            np.array([np.array(tool_pred), np.array(tool_true), np.array(tool_prob)]))

if __name__=='__main__':
    traintesttxt_dir = 'cholec80/traintest_txt'
    testtxtdir = os.path.join(traintesttxt_dir,'testswinnet.txt')
    modeldir = os.path.join('./pre-trained_model/toolnet/swinnet.pth')
