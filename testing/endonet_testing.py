...
def testing(testtxtdir, modeldir, savedir):
    ...
    test_data = EndonetDataset(txt_path_test, transform=transform_test)
    ...
    np.save(os.path.join(savedir,'endonet_results.npy'),
            np.array([tool_pred, tool_true, tool_prob]))
    np.save(os.path.join(savedir,'endonet_phase_results.npy'),
            np.array([phase_pred, phase_true, phase_prob]),
            allow_pickle=True)

traintesttxt_dir = 'cholec80/traintest_txt'
testtxtdir = os.path.join(traintesttxt_dir,'testendonet.txt')
modeldir = os.path.join('./pre-trained_model/endonet/endonet.pth')
