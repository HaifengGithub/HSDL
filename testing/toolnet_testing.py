...
import numpy as np
from utils.MyDataset import ToolnetDataset
import os
import torch
from model.toolnet import toolnet
...
def testing(testtxtdir, modeldir, savedir):
    ...
    test_data = ToolnetDataset(txt_path_test, transform=transform_test)
    ...
    np.save(os.path.join(savedir,'toolnet_results.npy'),
            np.array([np.array(tool_pred), np.array(tool_true), np.array(tool_prob)]))

if __name__=='__main__':
    traintesttxt_dir = 'cholec80/traintest_txt'
    testtxtdir = os.path.join(traintesttxt_dir,'testtoolnet.txt')
    modeldir = os.path.join('./pre-trained_model/toolnet/toolnet.pth')
