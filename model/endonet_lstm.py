...
class endonet_lstm(nn.Module):
    def __init__(self):
        super(endonet_lstm, self).__init__()
        ...
        self.fcphase = Sequential(
            nn.Linear(512 + 14, 512, bias=True),
            nn.ReLU(),
            # nn.Dropout(0.5),
    ...
    def forward(self, x):
        ...
        x6 = self.fc6(x)
        x7 = self.fc7(x)
        p = self.fcphase(torch.cat([x,x1,x2,x3,x4,x5,x6,x7],dim=1))
        return (x1, x2, x3, x4, x5, x6, x7, p)